...
class Kp extends MY_Controller {
	...
	function index(){
		...
		if($setuju != NULL){
			...
			$this->load->view('kp/kp_waiting',['waiting' => $waiting,'error' => '']);
	...
	public function upload_file(){
		//$this->KpModel->update_file();
		$config['upload_path']		= './upload/balasankp/';
		$config['allowed_types']	= 'pdf';
        $config['file_name']		= $this->input->post('id_kp');
		...
		if ( ! $this->upload->do_upload('dokumen')){
			...
			$this->load->view('kp/kp_waiting', ['waiting' => $waiting, 'error' => $error]);
		}else{
			$data = $this->upload->data();
			$nama = $data['file_name'];

			$update = $this->KpModel->update_file($nama);
			//var_dump($update);
			if($update){
				$this->session->set_flashdata('success', 'Upload Berkas Berhasil disimpan');
				redirect('pengajuankp');
			}else{
				$this->session->set_flashdata('success', 'Upload Berkas Gagal disimpan');
				redirect('pengajuankp');
			}
		}
	}
